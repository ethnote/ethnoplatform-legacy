...
export type CreatableSelectMultiField = FieldBase<'creatable_select_multi'> &
  HasOptions
/**
...
function CreatableSelectMultiInternal<T>(
  props: FormControlProps<T, CreatableSelectMultiField>,
) {
  return (
    <Control<T, string> {...props}>
      {(childProps) => {
        return (
          <CreatableSelect
            {...childProps}
            placeholder={props.options.placeholder ?? 'Select or create...'}
            size='md'
            useBasicStyles
            isMulti
            isClearable
            ...
            value={undefined}
            onChange={(e) =>
              childProps.onChange((e.map((x) => x?.value) as any) ?? [])
            }
          />
        )
      }}
